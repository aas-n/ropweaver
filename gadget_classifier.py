...
def classify_gadget(gadget, bad_bytes, virtualAddress, debug):
    classifications = []
    ...
    if ": " in gadget:
        ...
        # Linux format
        address, instructions = gadget.split("  # ", 1)
        # Standardize 'ret' and 'retn' variations to 'ret;' or 'ret <offset>;'
        instructions = re.sub(r"\b(retn?|ret)\s*(0x[0-9a-fA-F]+)?\s*;?", lambda m: f"ret {m.group(2)};" if m.group(2) else "ret;", instructions)
    ...
    if contains_bad_instructions(instructions):
        debug_print("[-] Bad instruction: " + address + " " + instructions, debug)
        return classifications
    if contains_relative_addressing(instructions):
        debug_print("[-] RelativeAddress: " + address + " " + instructions, debug)
        return classifications
    if has_more_push_than_pop(instructions):
        debug_print("[-] morePushThanPop: " + address + " " + instructions, debug)
        return classifications
    if contains_hardcoded_address(instructions):
        debug_print("[-] hardcodedAddress: " + address + " " + instructions, debug)
        return classifications
    if contains_bad_bytes(address, bad_bytes):
        debug_print("[-] badbytes: " + address + " " + instructions, debug)
        return classifications
...
def classify_gadgets(gadget_list, bad_bytes, virtualAddress, debug):
    categories = {
    ...
    for gadget in gadget_list:
        classifications = classify_gadget(gadget, bad_bytes, virtualAddress, debug)
